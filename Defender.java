public class Defender extends Player {
  ...
  private double originalSpeedX = Math.abs((Math.random() * 3.75 - 1)) ;
  private double originalSpeedY = Math.abs((Math.random() * 3.75 - 1)) ;
  ...
  public int catchBlues(Field field) {
    //Purpose: Tells red Defenders to pick a valid blue target and chase them down
    //Side-Effects: Returns 1, telling the player it shouldn't patrol anymore
    Player Target = null;
    
    for (int i = 0; i < field.getTeam1().size(); i += 1) {
      if ((((Player) field.getTeam1().get(i)).getX() > 600) && (((Player) field.getTeam1().get(i)).beenCaught == false) && (((Player) field.getTeam1().get(i)).beenFreed == false) ) {
        //Defenders stay back, waiting for enemies to get deep into their territory
        //Once they're there, the defenders start chasing
  ...
  public int catchReds(Field field) {
    //A mirrored version of catchBlues for blue Defenders
    Player Target = null;
    
    for (int i = 0; i < field.getTeam2().size(); i += 1) {
      if ((((Player) field.getTeam2().get(i)).getX() < 200) && (((Player) field.getTeam2().get(i)).beenCaught == false) && (((Player) field.getTeam2().get(i)).beenFreed == false) ) {
  ...
  public int patrol(int patrolsCalled){
    ...
    //Defenders are the back lines, so they patrol near the rear of their territory
    
    this.speedX = this.originalSpeedX;
    this.speedY = this.originalSpeedY;
    
    if (this.team.intern() == "reds") {
      distanceX = (700 - this.x);
    }
    else {
      distanceX = (100 - this.x);
    }
  ...
  public Defender(Field f, int side, String name, int number, String team,char symbol, double x, double y){
