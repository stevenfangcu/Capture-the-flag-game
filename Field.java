...
public class Field{
  ...
  double flag2OriginalX = 764;
  double flag2OriginalY = 401;
  double flag1OriginalX = 44;
  double flag1OriginalY = 201;
  ...
  public boolean catchOpponent(Player a, Player b){
    if( a.getTeam().intern() == b.getTeam().intern() ){ // checks team
      return false;
    }
      if( Math.hypot( a.getX() - b.getX(), a.getY() - b.getY() ) <= ARMS_LENGTH ){ // in arms length
        b.beenCaught = true;
        ...
        System.out.println(aName + " of the " + aTeam + " has caught " + bName + " of the " + bTeam + "!" );
        return true;
      }
  ...
  public boolean freeTeammate(Player a, Player b){
    //This is where beenFreed comes into play; when someone is freed, they get a free walk back to base
    //So beenFreed a) tells the freed player not to execute its regular behaviour until it's tagged its base
    //and b) tells opposing players not to try and catch them
    if( a.getTeam().intern() != b.getTeam().intern() ){ // checks team
      return false;
    }
    if( Math.hypot( a.getX() - b.getX(), a.getY() - b.getY() ) <= ARMS_LENGTH ){
      ...
      System.out.println(aName + " of the " + aTeam + " has freed teammate: " + bName + "!");
      return true;
    }
    return false;
  }
  public boolean pickUpFlag(Player a){
    ...
    if(Math.hypot( a.getX() - b.getX(), a.getY() - b.getY() ) <= ARMS_LENGTH){
      if(a.getTeam().intern() == "blues" && b.getY() == this.flag2OriginalY && b.getX() == this.flag2OriginalX ){
        String aTeam;
        aTeam = "red team"; 
        String person;
        person = a.getName();
        System.out.println(person + " from blue team has taken the flag from the " + aTeam + "!");
        return true;
      }else if(a.getTeam().intern() == "reds" && b.getY() == this.flag1OriginalY && b.getX() == this.flag1OriginalX ){
        String aTeam;
        aTeam = "blue team"; 
        String person;
        person = a.getName();
        System.out.println(person + " from red team has taken the flag from the " + aTeam + "!");
      return true;
    }
    }
    return false;
  ...
  /** attempt to win the game
    ...
    * and is with ARMS_LENGTH of its own base.  False otherwise.
    */
  public String getWinTeam(){ // a getter for the teamWinner string
    return teamWinner;
  }
  
  public boolean winGame(Player a){
    Entity b = base2;
    if( a.getTeam().intern() == "blues" ){
      b = base1;
    }
    
    if(( Math.hypot( a.getX() - b.getX(), a.getY() - b.getY() ) <= ARMS_LENGTH ) && (a.hasFlag == true) ){
      //No player who lacks the flag should ever attempt to win the game, but better safe than sorry
      GAME_IS_WON = true;
      String aTeam;
      if(a.getTeam() == "blues"){
        aTeam = "blue team"; 
      ...
      String person;
      person = a.getName();
      teamWinner = a.getTeam();
      System.out.println("That's game! " + person + " has won the game for the " + aTeam + "!");
      return true;
    }
    return false;
  }
  ...
  public Field(){
    ...
    flag1OriginalX = flag1.getX();
    flag1OriginalY = flag1.getY();
    ...
    flag2OriginalX = flag2.getX();
    flag2OriginalY = flag2.getY();
  ...
  /** attempt to pick up a flag
    ...
    * @return true if <code>a</code> is within ARMS_LENGTH of 
